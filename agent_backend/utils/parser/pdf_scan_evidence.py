...
def compact_recovery_geometry(attempts):
    """本页最终证据去重；仅共享完全相同的几何，不改变任何识别决策。"""
    sources = {}
    for index, attempt in enumerate(attempts):
        window = attempt.get('pixel_window')
        if not isinstance(window, (list, tuple)) or len(window) != 4:
            continue
        key = (tuple(window), attempt.get('recovery_dpi'),
               attempt.get('segmentation_psm'), tuple(attempt.get('ocr_input_size', ())))
        previous = sources.setdefault(key, [])
        for field in ('image_line_support', 'candidate_lines'):
            if field not in attempt:
                continue
            source = next((old for old in previous if field in attempts[old]
                           and attempts[old][field] == attempt[field]), None)
            if source is not None:
                attempt[field + '_attempt_index'] = source
                del attempt[field]
        previous.append(index)


def recovery_geometry(attempts, index, field):
    """兼容旧内联字段；新引用仅允许指向较早的同页尝试。"""
    if not isinstance(index, int) or not 0 <= index < len(attempts):
        return []
    item = attempts[index]
    if field in item:
        return item[field]
    source = item.get(field + '_attempt_index')
    if type(source) is int and 0 <= source < index:
        return attempts[source].get(field, [])
    return []
