...
class PreReviewSubmissionService:
    ...
    def _enrich_payload_with_remote_appendices(
        ...
        *,
        ...
        if not changed:
            # 显式保存空列表作为附录增强已完成的标记；否则后续每次读取都会把
            # 没有附录的正常文档误判为旧数据，并重复扫描和同步数据库。
    ...
    def get_submission_content(
        ...
        try:
            ...
            if project is not None and self.owner._is_ctd_structure_project(project):
                ...
                if compact and not bool(payload.get("content_ready", True)):
                    return False, "submission content not ready; reparse required", None
        ...
        if compact:
            ok_payload, payload_msg, saved_payload = self.read_submission_parsed_payload(
                project_id=project_id,
                doc_id=doc_id,
            )
            if not ok_payload:
                return False, payload_msg, None
            section_rows = self.owner._extract_section_content_rows_from_payload(saved_payload)
            compact_payload = self.owner._compact_structured_project_payload(
                {
                    "sections": section_rows,
                    "statistics": {
                        "section_total": len(section_rows),
                        "review_unit_total": (
                            len(saved_payload.get("review_units", []))
                            if isinstance(saved_payload, dict)
                            and isinstance(saved_payload.get("review_units", []), list)
                            else len(section_rows)
                        ),
                    },
                }
            )
            return True, "success", {
                ...
                "project_id": project_id,
                "chunk_size": int(
                    compact_payload.get("statistics", {}).get("review_unit_total", 0)
                    or 0
                ),
    ...
    def get_submission_sections(
        ...
        if compact:
            ok, msg, payload = self.read_submission_parsed_payload(
                project_id=project_id,
                doc_id=doc_id,
            )
        else:
            ok, msg, payload = self.load_submission_parsed_payload(
                project_id=project_id,
                doc_id=doc_id,
            )
        if not ok:
    ...
    def get_submission_section_overview(
        ...
        该路径只在 SQL 中聚合字符数并读取已持久化的短摘要，不取回
        content 长正文，也不读解析 JSON、不调用 Word/PDF/OCR 解析。
        """
        normalized_project_id = str(project_id or "").strip()
        normalized_doc_id = str(doc_id or "").strip()
        if not normalized_project_id or not normalized_doc_id:
            return False, "project_id/doc_id is required", None
        ...
        try:
            ...
            if self.owner._is_ctd_structure_project(project):
                catalog = self.owner._load_project_section_catalog(session, normalized_project_id)
            catalog_sections = catalog.get("all_sections", []) if isinstance(catalog, dict) else []
            if not isinstance(catalog_sections, list) or not catalog_sections:
                catalog_sections = catalog.get("flat_sections", []) if isinstance(catalog, dict) else []
            if not isinstance(catalog_sections, list):
                catalog_sections = []
            ...
            try:
                ...
                dialect_name = ""
            content_length = (
                func.char_length(PreReviewSubmissionSectionContent.content)
                if dialect_name in {"mysql", "mariadb"}
                else func.length(PreReviewSubmissionSectionContent.content)
            )
            aggregate_rows = (
                session.query(
                    PreReviewSubmissionSectionContent.section_id,
                    func.max(PreReviewSubmissionSectionContent.section_code),
                    func.max(PreReviewSubmissionSectionContent.section_name),
                    func.sum(content_length),
                    func.max(PreReviewSubmissionSectionContent.content_preview),
                )
                .filter(
                    and_(
                        PreReviewSubmissionSectionContent.project_id == normalized_project_id,
                        PreReviewSubmissionSectionContent.doc_id == normalized_doc_id,
                    )
                )
                .group_by(PreReviewSubmissionSectionContent.section_id)
                .all()
            )
            content_meta_by_section: Dict[str, Dict[str, Any]] = {}
            for section_id, section_code, section_name, char_count, content_preview in aggregate_rows:
                normalized_section_id = str(section_id or "").strip()
                if not normalized_section_id:
                    continue
                content_meta_by_section[normalized_section_id] = {
                    "section_code": str(section_code or normalized_section_id).strip() or normalized_section_id,
                    "section_name": str(section_name or "").strip(),
                    "char_count": int(char_count or 0),
                    "content_preview": str(content_preview or "").strip()[:320],
                }

            sections: List[Dict[str, Any]] = []
            section_ids = set()
            for catalog_item in catalog_sections:
                if not isinstance(catalog_item, dict):
                    continue
                section_id = str(catalog_item.get("section_id", "") or "").strip()
                if not section_id or section_id in section_ids:
                    continue
                section_ids.add(section_id)
                content_meta = content_meta_by_section.get(section_id, {})
                sections.append(
                    {
                        "section_id": section_id,
                        "section_code": str(
                            catalog_item.get("section_code", "")
                            or content_meta.get("section_code", "")
                            or section_id
                        ).strip()
                        or section_id,
                        "section_name": str(
                            catalog_item.get("section_name", "")
                            or catalog_item.get("title", "")
                            or content_meta.get("section_name", "")
                            or section_id
                        ).strip()
                        or section_id,
                        "title_path": list(catalog_item.get("title_path") or [])
                        if isinstance(catalog_item.get("title_path", []), list)
                        else [],
                        "parent_section_id": str(catalog_item.get("parent_section_id", "") or "").strip(),
                        "char_count": int(content_meta.get("char_count", 0) or 0),
                        "content_preview": str(content_meta.get("content_preview", "") or ""),
                    }
                )
            for section_id, content_meta in content_meta_by_section.items():
                ...
                sections.append(
                    {
                        "section_id": section_id,
                        "section_code": str(content_meta.get("section_code", "") or section_id),
                        "section_name": str(content_meta.get("section_name", "") or section_id),
                        "title_path": [],
                        "parent_section_id": "",
                        "char_count": int(content_meta.get("char_count", 0) or 0),
                        "content_preview": str(content_meta.get("content_preview", "") or ""),
            ...
            return True, "success", {
                ...
                "statistics": {
                    ...
                    "total_char_count": sum(int(item.get("char_count", 0) or 0) for item in sections),
                },
                "content_ready": bool(content_meta_by_section),
                "response_mode": "overview_metadata",
            }
        finally:
            session.close()

    def get_submission_section_diagnostics(self, project_id: str, doc_id: str) -> Tuple[bool, str, Optional[Dict[str, Any]]]:
        ok, msg, data = self.get_submission_sections(project_id=project_id, doc_id=doc_id, compact=True)
        if not ok or not isinstance(data, dict):
            return ok, msg, data
        ...
        if not parse_quality and not parser_warnings:
            # 诊断属于只读接口，只在内存已有解析结果时读取；这里绝不能触发解析
            # 或同步路径。
    ...
    def read_submission_parsed_payload(
        self,
        project_id: str,
        doc_id: str,
    ) -> Tuple[bool, str, Any]:
        """只读取已持久化的解析结果，不执行解析或网络请求。

        章节浏览和诊断都是只读操作，不能仅因旧缓存缺失就启动 Word/PDF 解析、
        DeepSeek OCR 或远程附录下载。解析只能由上传流程或显式重解析任务执行。
        """
        normalized_project_id = str(project_id or "").strip()
        normalized_doc_id = str(doc_id or "").strip()
        if not normalized_project_id or not normalized_doc_id:
            return False, "project_id/doc_id is required", {}

        session = self.owner.db_conn.get_session()
        try:
            exists = (
                session.query(PreReviewSubmissionFile.id)
                .filter(
                    and_(
                        PreReviewSubmissionFile.project_id == normalized_project_id,
                        PreReviewSubmissionFile.doc_id == normalized_doc_id,
        ...
        cache_key = self.owner._submission_payload_cache_key(
            normalized_project_id,
            normalized_doc_id,
        )
        cached_payload = self.owner._submission_payload_cache.get(cache_key)
        if isinstance(cached_payload, (dict, list)):
            return True, "ok", deepcopy(cached_payload)

        parsed_path = os.path.join(
            self.owner.SUBMISSION_PARSED_DIR,
            f"{normalized_doc_id}.json",
        )
        ...
        try:
            ...
            return False, f"submission parse result is unreadable: {str(exc)}", {}
        if not isinstance(payload, (dict, list)):
            return False, "submission parse result is invalid; reparse required", {}
