...
class PreReviewService:
    ...
    @staticmethod
    def _structured_project_payload_cache_key(project_id: str, doc_id: str) -> str:
        return f"{str(project_id or '').strip()}::{str(doc_id or '').strip()}"

    @staticmethod
    def _compact_structured_project_payload(payload: Dict[str, Any]) -> Dict[str, Any]:
        """Return the read-only chapter-content DTO used by the browser.

        The full structured payload intentionally contains review units,
        attachments, paragraph anchors and a content-decorated chapter tree for
        the review pipeline. Sending that object to the chapter browser repeats
        the same long text many times, so the browser endpoint only receives the
        two text variants it can actually display.
        """
    ...
    def _invalidate_structured_project_payload_cache(self, project_id: str = "", doc_id: str = "") -> None:
        ...
        if project_key and doc_key:
            self._structured_project_payload_cache.pop(
                self._structured_project_payload_cache_key(project_key, doc_key),
                None,
            )
            return
        if project_key:
    ...
    def _normalize_ctd_payload_to_leaf_sections(self, payload: Any, root_section_id: str) -> Any:
        ...
        # A source document may place body text directly on an intermediate CTD node
        # while the configured catalog defines several more-specific leaves.
        # There is no honest way to choose one child in that case. Keep the
        # exact source chapter as a displayable and replayable unit instead of
        # silently dropping its content during leaf normalization.
    ...
    def _compute_project_module_upload_status(self, session, project_id: str) -> Dict[str, Any]:
        catalog = self.ctd_sections.get_catalog()
        roots = catalog.get("chapter_structure", []) if isinstance(catalog, dict) else []
        ...
        for root in roots:
            ...
            uploaded_count = len([sid for sid in module_leaves if sid in uploaded_leaf_ids])
                ...
                "list": [
                    {
                        ...
                        **self._compute_project_module_upload_status(session, r.project_id),
    ...
    def _build_structured_project_payload(
        ...
        if source_doc_key:
            cache_key = self._structured_project_payload_cache_key(project_id, source_doc_key)
            cached_payload = self._structured_project_payload_cache.get(cache_key)
            if isinstance(cached_payload, dict):
                return (
                    self._compact_structured_project_payload(cached_payload)
                    if compact
                    else deepcopy(cached_payload)
                )
        catalog = self._merge_catalog_with_manual_concerns(
            self._load_project_section_catalog(session, project_id),
        ...
        # Content is parsed against the headings that actually exist in the
        # source document. A Word file may legitimately contain text on an
        # intermediate CTD node (for example 3.2.p.2) while the configured
        # catalog also defines deeper leaves (3.2.p.2.1 ... 3.2.p.2.6). Using
        # leaf nodes only makes the correctly parsed parent text disappear from
        # both the UI and replay diagnostics. Bind display/replay content across
        # the complete catalog hierarchy; leaf-only data remains available in
        # `flat_sections` for workflows that explicitly need leaves.
        ...
        submission_file_query = session.query(PreReviewSubmissionFile).filter(
            and_(
                PreReviewSubmissionFile.project_id == project_id,
                PreReviewSubmissionFile.is_deleted == 0,
                PreReviewSubmissionFile.section_id.isnot(None),
        ...
        if source_doc_key:
            submission_file_query = submission_file_query.filter(PreReviewSubmissionFile.doc_id == source_doc_key)
        rows = submission_file_query.order_by(PreReviewSubmissionFile.id.asc()).all()
        ...
        for row in rows:
            ...
            ok_payload = False
            payload: Any = None
            if not text_blocks and allow_parse_fallback:
        ...
        for index, base in enumerate(content_sections, start=1):
            ...
            section_rules = self._normalize_text_list(base.get("section_rules", []))
            raw_blocks: List[str] = []
            ...
            if merged_raw_text and not compact:
                review_units.append(
                    {
                        "chunk_id": sid,
                        "section_id": sid,
        ...
        payload = {
            ...
            "statistics": {
                ...
                "review_unit_total": (
                    len(review_units)
                    if not compact
                    else sum(1 for item in sections if str(item.get("raw_content", "") or "").strip())
                ),
                "attached_file_total": sum(len(v) for v in attachments_by_section.values()),
            },
        }
        if source_doc_key and not compact:
            self._structured_project_payload_cache[cache_key] = deepcopy(payload)
        return self._compact_structured_project_payload(payload) if compact else payload
        ...
        try:
            ...
            for r in rows:
                ...
                conclusion_rows = (
                    ...
                    .filter(PreReviewSectionConclusion.run_id == r.run_id)
                    .all()
                )
                ...
                feedback_rows = (
                    session.query(PreReviewFeedback.section_id, func.count(PreReviewFeedback.id))
                    .filter(PreReviewFeedback.run_id == r.run_id)
                    .group_by(PreReviewFeedback.section_id)
                    .all()
                )
                for sid, count in feedback_rows:
                    section_id = str(sid or "").strip()
                    if not section_id:
                        continue
                ...
                if current_run is not None:
                    ...
                    for previous_run in previous_runs:
                        previous_rows = (
                            ...
                            .filter(
                                PreReviewExecutionAudit.run_id == str(previous_run.run_id or "").strip(),
    ...
    def _build_review_conclusion_export_context(
        ...
        project = (
            session.query(PreReviewProject)
            .filter(PreReviewProject.project_id == run.project_id)
            .first()
        )
    ...
    def _build_feedback_history(self, session, run_id: str, section_id: str = "") -> Dict[str, Any]:
        ...
        optimize_events = []
        signal_entries = []
        for payload in self._load_feedback_analysis_payloads(session, run_id=run_id, section_id=section_id):
    ...
    def get_feedback_stats(self, run_id: str, section_id: str = "") -> Tuple[bool, str, Optional[Dict[str, Any]]]:
        ...
        try:
            ...
            history = self._build_feedback_history(session, run_id=run_id, section_id=section_id)
