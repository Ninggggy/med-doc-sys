...
def ocr_region(page, rect, client, lang, dpi, psm, covered_words=(), *, original=False, deadline=None, metadata=None, raw_scan=False):
    scale = dpi / 72
    ...
    for i, raw in enumerate(data.get('text', [])):
        ...
        box = [(pix.x+x)/scale, (pix.y+y)/scale, (pix.x+x+w)/scale, (pix.y+y+h)/scale]
        ...
        if re.search(r'\d', text) or matches:
            ...
            evidence['bbox_pdf'] = [(pix.x+pixel_box[0])/scale, (pix.y+pixel_box[1])/scale,
                                    (pix.x+pixel_box[2])/scale, (pix.y+pixel_box[3])/scale] if valid_box else box
...
def recover_ocr_coverage(page, rect, client, lang, dpi, words, deadline, metadata, *, quality_lines=()):
    ...
    for index, (ink_box, crop, trigger) in enumerate(jobs):
        ...
        if trigger == 'ocr_quality' and metadata.get('recovery_cells') and len(owners) != 1:
            ...
            continue
        mode = layout_choice['mode'] if layout_choice else (7 if trigger == 'ocr_quality' or ink_box.width > ink_box.height * 2 else 6)
        recovery_dpi = layout_choice['dpi'] if layout_choice else dpi
        attempt['recovery_dpi'] = recovery_dpi
        # 只复用本次区域内完全相同的图像与分段方式；缺失证据仍逐项判断。
        crop_key = (tuple(crop), mode if not reflow_cell else 'cell_reflow', recovery_dpi)
        planned.append((index,ink_box,crop,trigger,mode,recovery_dpi,reflow_cell,crop_key,attempt))
    # 先按合并后的诊断面积分配四个名额，再按原阅读顺序执行。
    # 面积仅用于预算调度，不改变置信度、遗漏状态或人工确认要求。
    groups = {}
    for index,ink_box,_,_,_,_,_,crop_key,_ in planned:
    ...
    selected = set(sorted(groups,key=lambda key:(
        -groups[key]['priority'], -sum(fitz.Rect(box).get_area() for box in groups[key]['boxes']),groups[key]['first']))[:4])
    for index,ink_box,crop,trigger,mode,recovery_dpi,reflow_cell,crop_key,attempt in planned:
        ...
        if cached is None and time.monotonic() >= deadline:
            ...
            continue
        if cached is None and (crop_key not in selected or len(crop_results) >= 4):
        ...
        try:
            ...
            for candidate_index, word in enumerate(processing_candidates):
                ...
                def ordered_neighbor(old):
                    ...
                    if (current_lineage is None or previous is None or previous[0] != crop_key
                            ...
                            or abs(previous[1][1]-current_lineage[1]) != 1
                ...
                if not comparable or (len(word['text'].strip()) == 1 and not any(
            ...
            for group in word_lines(accepted):
    ...
    with fitz.open(file_path) as doc, pdfplumber.open(file_path) as plumber, ExitStack() as normalized_docs:
        for index in range(len(doc)):
            ...
            try:
                ...
                embedded_words = []
                if native.strip() and image_coverage > .65:
                ...
                if embedded_words:
                    ...
                    try:
                        ...
                        row['embedded_table_candidates'] = [
                            {'bbox_pdf':list(fitz.Rect(t.bbox)*transform),
                             'cells':[list(fitz.Rect(c)*transform) for c in t.cells]}
                ...
                for rect in regions:
                    ...
                    try:
                        ...
                        if ow or budget_metadata.get('execution_budget_enforced'):
                            ...
                            row['ocr_calls'] += sum(a['status'] != 'not_attempted'
                        ...
                        if embedded_words:
                            ...
                            row['errors'].extend(finalize_embedded_evidence(evidence,attempts))
