...
def finalize_embedded_evidence(evidence, attempts):
    ...
    for entry in evidence:
        ...
        for _,attempt in relevant:
            ...
            safe=lambda ws: bool(ws) and all(w.get('confidence',0)>=80 and w.get('numeric_verification',{}).get('status','verified')=='verified' for w in ws)
        ...
        if entry['status'] not in ('matched','revalidated'):
            errors.append({'stage':'embedded_ocr','code':entry['trigger'], 'bbox_pdf':list(box),
                ...
                'embedded_evidence_index':evidence.index(entry)})
    return errors
